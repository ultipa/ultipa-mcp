...
export const SERVER_INSTRUCTIONS = [
  ...
  "",
  "For Cloud instances, the `adminPassword` is NOT returned by `list_instances` or `get_instance`. It's returned by `create_instance` — surface it to the user immediately after a create. For an existing instance whose password is unknown, call `get_instance_credentials` (requires `ULTIPA_CLOUD_API_KEY` to have the `instances:credentials` permission).",
  "",
  ...
  "a. An instance can host multiple graphs (check via `list_graphs`) with one marked as 'current', and queries run on it. Before running a query, ensure you know which graph the user is now working with.",
  ...
  "- **Built-in algorithms** (run via `run_algo`): `lookup_docs('graph-algorithms/pages/introduction')` for the algo catalog by category, then `lookup_docs('graph-algorithms/pages/<category>/<algorithm>')` for one algorithm details (e.g. `centrality/pagerank`, `community-detection/louvain`). If needs to further process algo results, you may combine algo with other GQL statements and run via `run_gql_query`.",
  "- **Stored procedures**: to call a procedure, use `run_gql_query` with `CALL <procedureName>(...)` (list available with `SHOW PROCEDURES`; see `lookup_docs('stored-procedures/pages/calling-procedures')`). To **create** a procedure, use `write_procedure` — the procedure body has its own grammar (NOT GQL), so always `lookup_docs('stored-procedures/pages/procedure-body/procedure-body-language')` first.",
  "",
  ...
  "Before calling `import_data` you MUST print this exact preview block and STOP:",
  "```",
  "Import plan",
  "Graph: <name>",
  ...
  "  - <label> × ~<count>",
  "    _id ← <source column>",
  "    props: <src>→<prop>, <src>→<prop>, ...",
  "Edges:",
  "  - <label> × ~<count>",
  "    _from (refs node _id) ← <source column>",
  "    _to (refs node _id) ← <source column>",
  "    props: <src>→<prop>, ...",
  "```",
  "Then ask `Reply 'go' to execute, or specify corrections.` and WAIT for the user's reply. **Printing the plan is NOT confirmation.** Do NOT call `import_data` until the user explicitly replies 'go' or equivalent",
  "",
  "If user has **very large imports** (millions of rows) or **other data sources** (SQL databases, other graph platforms, big-data systems) → suggest Ultipa Manager → Data Integration (UI wizard, built into Manager — Cloud instances open via `managerUrl`) or **Ultipa Transporter** (batch ETL CLI, https://www.ultipa.com/docs/tools/transporter).",
  "",
